...
router.get("/", (req, res) => {
    ...
    .then((course) => {
        res.send(course);
    }).catch(() => {res.status(500).send("Error !! Cannot get course!!");
    });
});
...
/*/ 自己打的, postCouse頁面顯示不出來
router.get("/instructor/:_instructor_id", (req, res) => {
    let { _instructor_id } = req.params;
    Course.find({ instructor: _instructor_id }).populate("instructor", ["username", "email",])
    .then((data) => {
        res.send(data);
    }).catch(() => {
        res.status(500).send("Connot get course data.");
    });
...
router.get("/findByName/:name", (req, res) => {
    let { name } = req.params;
    Course.find ({ title: name }).populate("instructor", ["username", "email"])
    .then((course) => {
...
router.get("/student/:_student_id", (req, res) => {
    ...
    Course.find({ students: _student_id }).populate("instructor", ["username","email",
    ]).then((courses) => {
...
router.post("/", async (req, res) => {
    ...
    //const { error} = courseValidation(req,body); chat gpt修正如下方
